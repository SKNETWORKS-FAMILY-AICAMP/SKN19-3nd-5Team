...
|전처리|내용|
|:---:|---|
|파이프라인|원본 데이터 → LLM을 이용한 추가 정보 생성 및 병합 → RAG 활용|
|목적|식물에 대한 메타데이터 생성|
...
RAG 검색 결과를 참고해서 사용자에게 식물 1가지를 추천하세요.
이미 추천한 식물은 사용자가 거부한 식물이니 추천하지 않습니다.
사용자 정보 중 비정상적인 항목이 있는 경우 먼저 해당 항목을 명시한 후, 무시하고 추천합니다.
절대로 허위사실을 작성하지 마세요.
색을 언급할 때는 코드가 아닌 한글 색상명을 사용하세요.
...
1️⃣ 과도한 재추천 시 할루시네이션 발생
  - → RAG 검색 로직 개선: 반복 질의 시 검색 범위를 확대

2️⃣ 선물(꽃다발) 관련 데이터 부족으로 추천에 어려움
  - → 꽃다발, 포장, 선물 상황 등 전문적인 데이터를 확보
  - → 추가 수집이 어려울 경우, LLM을 선물 및 꽃 관련 도메인에 특화하여 파인튜닝 계획

2️⃣ 엉뚱한 입력을 해도 추천해주는 현상
  - 정밀한 프롬프트 예외 처리 필요
  - → 추가 데이터 수집 및 ReRanking 적용
  - → BAAI/bge-reranker-large 모델을 한국어 데이터로 파인튜닝한 'Dongjin-kr/ko-reranker'

<div align="left"> 
<img src="https://github.com/user-attachments/assets/a4c264a1-a00f-465d-865a-619111628eb0" width="80%" alt=" " style="float: left; margin: 0; padding: 0;" />
<br style="clear: both;" /> 
